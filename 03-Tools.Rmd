...
As we have already pointed out, you could start programming in the default text editor of your operating system, however take our word on that: You do not want to do that. Most developers use an Integrated Development Environment (IDE) to write code because it comes with a lot of handy features that help developers code faster and cleaner.

They come packed full of features, most of which you wont need for this course, however once you get used to the environment, you will never look back. Let's look at some reasons they're so handy.
...
For this course and your project, you will use the online code editor [CodeSandbox](https://codesandbox.io/). You will learn exactly how that is done in the first Meetup. This IDE is based on the same platform as Visual Studio Code, but offers some additional functionalities. The user interface has been designed in a way that allows you to see your file directory, editor, and your executed program all at once. CodeSandbox allows you to create teams that can not only access the same files but also make it possible to have live coding session together. To sign up you only need a free Github account, which you can create in a matter of seconds.

## API

For this semester project you will also need to learn about APIs. But what exactly is an API? An API (Application Programming Interface) allows different devices to connect to each other and send, update and request data from one to the other. You can think of it as a waiter in a restaurant. The waiter connects you to the kitchen, takes and understands your order, delivers it to the kitchen and comes back with the very thing you desired. APIs function ina very similar way.  
...
In JavaScript we use the [Fetch API](https://developer.mozilla.org/de/docs/Web/API/Fetch_API) to make our API call.  You will learn more about how to use it in [section 28](https://www.udemy.com/course/the-web-developer-bootcamp/learn/lecture/22051352#overview) of the Udemy course which will also go over [JSON][JSON], but to provide you with a template for later on and as a reminder, this is one way of how to deal with fetch:
```{javascript}
...
Beware, this course filled with a ton of content. For our project we are only focusing on the front-end so you can naturally ignore (for now) the later half of the course, which exclusively deals with the back-end. We encourage you to do all the exercises you will encounter in between. You will learn a lot by actually doing the exercises and programming on your project will become far simpler.
